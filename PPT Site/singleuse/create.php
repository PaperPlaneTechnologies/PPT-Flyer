<?php
	...
	require_once("Applications/MAMP/htdocs/singleuse/variables.php");
...
// Get a human readable file size from bytes
	...
	function generate() {
		include("Applications/MAMP/htdocs/singleuse/variables.php");
		...
		if(is_array($PROTECTED_DOWNLOADS)) {
			foreach ($PROTECTED_DOWNLOADS as $key => $download) {
				 ...
				 */
				if(!is_dir('Applications/MAMP/htdocs/singleuse/keys')) {
					mkdir('Applications/MAMP/htdocs/singleuse/keys');
					$file = fopen('Applications/MAMP/htdocs/singleuse/keys/.htaccess','w');
				 ...
				 */
				$file = fopen('Applications/MAMP/htdocs/singleuse/keys/keys','a');
				fwrite($file,"{$new}\n");
				fclose($file);
